...
For binary work, Planner defines Research questions and evidence exit conditions, not recovered answers. Research records evidence-backed ABI/layout/protocol/state/flow/algorithm/cryptography/security findings as `recovered_specs`; an accepted dependency is projected directly to Engineering without Planner transcription. Confidence describes evidence strength, never a project requirement. Only a separate project decision may promote an accepted finding into a required Engineering `interface_spec`.
...
| Direct Control answer | User text, control state or an already verified semantic summary is sufficient; collect no new domain evidence | Retained Owner query if new domain evidence is required |
| Retained Owner answer-only query | One bounded domain answer needs new evidence, no mutation/progression is requested, and an exact same-WP Owner exists; use `query --wp <WP> --question <text>` | Use Control/Owner-work; never disguise work as a query or repeat it |
