...
class EngineeringAndIntegrity(Base):
    ...
    def test_cli_doctor_makes_no_live_claim(self):
        ...
        self.assertEqual(rc,0);v=parse(out.getvalue().encode());self.assertEqual(v['version'],'1.0.1');self.assertFalse(v['live_orca_verified']);self.assertEqual(v['model_calls'],0)
